...
## 📋 2026-05-18 全面盘点（22 项 + 优先级矩阵）

> 这次系统扫了一遍能优化的地方，按"价值 × 难度"分类。下面 5 大类（立即/中期/长期/工程债/安全）共 22 个待办。
> 旧版分级（高优/中优/低优）保留在下面历史区，不再用作主导。

### 🟢 立即见效（高 ROI，半天内做完）

| # | 优化项 | 解决的问题 | 工时 | 状态 |
|---|--------|----------|------|------|
| 1 | 「正在处理…」即时回复 | pipeline 跑 30-90 秒主人干等没反馈 | 30 min | ⏳ |
| 2 | 复习问题引入项目档案上下文 | 反思 3 问偏泛，关联项目卡点会更扎心 | 1 h | ⏳ |
| 3 | 批量 demote / promote (`digest demote 1,2,3`) | 整理库时一条一条搞太烦 | 30 min | ⏳ |
| 4 | 标签合并 `digest tag-merge "A" "B"` | tags 表会膨胀（同义词、近义词）| 30 min | ⏳ |
| 5 | 每日健康检查 cron + 告警 | 服务挂了/磁盘满了/token 过期主人不知道 | 1 h | ⏳ |

### 🟡 中期能力扩展（1-3 天）

| # | 优化项 | 价值 | 状态 |
|---|--------|------|------|
| 6 | B站官方字幕优先（不再 whisper 转录）| 速度 30s → 3s，准确率 90% → 100% | ⏳ |
| 7 | 微信公众号文章抓取（playwright + mp.weixin.qq.com）| 公众号是主流入口 | ⏳ |
| 8 | PDF 入库（论文/电子书 → 切片 → pipeline）| 攒了好多 PDF 没消化 | ⏳ |
| 9 | 音频笔记（你录语音 → whisper → text）| 走路时灵感记录 | ⏳ |
| 10 | iOS 快捷指令一键分享（任意 APP → 知识助手）| 最自然的入口 | ⏳ |
| 11 | 学习计划生成（综合 due + 项目档案 + 积压）| "本周建议消化 id=5,8,12" | ⏳ |

### 🔵 长期价值（数据积累后做）

| # | 优化项 | 何时做 | 状态 |
|---|--------|--------|------|
| 12 | 跨条关联推荐（入库时找最相关老 5 条）| 库存 ≥ 50 条 | ⏳ |
| 13 | 答题升级 - 复述模式（你说一段，AI 对比原文评价）| 你愿意用语音时 | ⏳ |
| 14 | 行为感知动态调度（不固定 9/13/21）| 数据攒 ≥ 2 周 | ⏳ |
| 15 | 主动洞察（"你 30 天没收 quant 了，项目搁置了吗"）| 数据攒 ≥ 1 个月 | ⏳ |

### ⚫ 工程债（不性感但必须）

| # | 优化项 | 何时做 | 状态 |
|---|--------|--------|------|
| 16 | **SQLite + items/ 定时备份到云存储**（强烈建议尽快做）| 现在所有数据都在一台机，挂一次全没 | 🔴 紧急 |
| 17 | logs 轮转 + 归档（不然写满磁盘）| 跑半年后必做 | ⏳ |
| 18 | 核心逻辑 pytest（SM-2、评分、查重）| 改算法时怕改坏 | ⏳ |
| 19 | `digest reindex` 兜底（从 md 重建 SQLite）| DB 坏了的兜底 | ⏳ |

### 🚨 安全（不解决以后会后悔）

| # | 隐患 | 解决方案 | 状态 |
|---|------|---------|------|
| 20 | DeepSeek API 限速没做 → 某天疯狂发图烧 100 块 | 加 daily token 上限 + 飞书告警 | ⏳ |
| 21 | 审计日志缺失 → archive 了 30 条不知道谁啥时候改的 | 加 audit_log 表，记 destructive 操作 | ⏳ |
| 22 | 飞书 token 每次重拿（虽能用但浪费）| 加缓存，2 小时刷一次 | ⏳ |
...
| 阶段 | 包含 | 总工时 |
|------|------|--------|
| **本周做完** | #1 + #2 + #5 + **#16** | 4-5 小时 |
| **本月做完** | #6 + #10 + #20 | 1-2 天 |
| **数据攒起来再做** | #12 / #14 / #15 | 等数据 |

**最该立刻做的两件：**
- 🥇 **#16 备份机制**：不做这事，前面所有努力随时归零
- 🥈 **#1 + #2 即时回复 + 项目档案问题**：1.5 小时解决两个最大体验痛点
